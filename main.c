...
printf("▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒\n\n"); 
  ...
  while(fgets(datosmov,1024,movcsv)!=NULL){
     Movimiento * mov = (Movimiento*) malloc (sizeof(Movimiento));
      ...
      mov->tipo_daño= get_csv_field(datosmov,4);
     insertMap(mapaMovimientos,mov->movimiento,mov);
     totalMovs++;
    if (totalMovs==67) break;
  } 
  ...
  //Ciclo donde comienza el combate con los pokemons que estan en el campo
    while(1){
      ...
      if ((pokeJugador->stats->vel > pokeEnemigo->stats->vel)){
        ...
        //Formula de daño "dano_total", F para daño Fisico, E para daño Especial
        ...
        if (strcmp(movi->tipo_daño,"F")==0){//Fisico
        dano_total=(((((2*100/7)*pokeEnemigo->stats->atk * movi->daño/pokeJugador->stats->def)/50) + 2)*coincideTipo(pokeEnemigo->tipo1,pokeEnemigo->tipo2,movi->tipo)*vsTipoRival1(listaEfectividad,movi->tipo,pokeJugador->tipo1)*vsTipoRival2(listaEfectividad,movi->tipo,pokeJugador->tipo2)*((rand()%15)+85)/100)*critico();
        } else if (strcmp(movi->tipo_daño,"E")==0){ //Especial
          dano_total=(((((2*100/7)*pokeEnemigo->stats->atk_esp * movi->daño/pokeJugador->stats->def_esp)/50) + 2)*coincideTipo(pokeEnemigo->tipo1,pokeEnemigo->tipo2,movi->tipo)*vsTipoRival1(listaEfectividad,movi->tipo,pokeJugador->tipo1)*vsTipoRival2(listaEfectividad,movi->tipo,pokeJugador->tipo2)*((rand()%15)+85)/100)*critico();
        }
...
printf("Cargando modo Multiplayer....\n");
  ...
  printf("Jugador 2, ingrese los nombres de los 3 pokémon aliados a elegir\n"); 
  //POKEMON NUMERO 1 
  while(1){
    printf("Pokemon 1 : "); scanf("%s",nombre_pokemon); 
    //strcpy(nombre_pokemon,"Arcanine");
  ...
  printf("\nEquipo Jugador 1-> %s | ",equipoJugador1[0]); printf("%s | ",equipoJugador1[1]); printf("%s",equipoJugador1[2]);
  printf("\nEquipo Jugador 2-> %s | ",equipoJugador2[0]); printf("%s | ",equipoJugador2[1]); printf("%s \n\n",equipoJugador2[2]);
  ...
  //Ciclo donde comienza el combate con los pokemons que estan en el campo
    while(1){
      ...
      if ((pokeJugador1->stats->vel > pokeJugador2->stats->vel)){
        ...
        printf("%s del Jugador 1 usa %s.\nLa salud de %s del Jugador 2 pasa de %d a ",pokeJugador1->nombre,movi->movimiento,pokeJugador2->nombre,pokeJugador2->stats->hp);
        ...
        //Segun el movimiento elegido, se buscara en el mapa para tenerlo a mano
        if (mov_seleccionado==1){
          movi =searchMap(mapaMovimientos,pokeJugador2->movs->mov1);
        }else if (mov_seleccionado==2){
          movi =searchMap(mapaMovimientos,pokeJugador2->movs->mov2);
        }else if (mov_seleccionado==3){
          movi =searchMap(mapaMovimientos,pokeJugador2->movs->mov3);
        }else if (mov_seleccionado==4){
          movi =searchMap(mapaMovimientos,pokeJugador2->movs->mov4);
        }
        
        //Formula de daño "dano_total", F para daño Fisico, E para daño Especial, X para no_type
        if (strcmp(movi->tipo_daño,"F")==0){//Fisico
        dano_total=(((((2*100/7)*pokeJugador2->stats->atk * movi->daño/pokeJugador1->stats->def)/50) + 2)*coincideTipo(pokeJugador2->tipo1,pokeJugador2->tipo2,movi->tipo)*vsTipoRival1(listaEfectividad,movi->tipo,pokeJugador1->tipo1)*vsTipoRival2(listaEfectividad,movi->tipo,pokeJugador1->tipo2)*((rand()%15)+85)/100)*critico();
        } else if (strcmp(movi->tipo_daño,"E")==0){ //Especial
            dano_total=(((((2*100/7)*pokeJugador2->stats->atk_esp * movi->daño/pokeJugador1->stats->def_esp)/50) + 2)*coincideTipo(pokeJugador2->tipo1,pokeJugador2->tipo2,movi->tipo)*vsTipoRival1(listaEfectividad,movi->tipo,pokeJugador1->tipo1)*vsTipoRival2(listaEfectividad,movi->tipo,pokeJugador1->tipo2)*((rand()%15)+85)/100)*critico();
        }

        //Se ve cuanto daño recibe Jugador 1 
        printf("%s del Jugador 2 usa %s.\nLa salud de %s del Jugador 1 pasa de %d a ",pokeJugador2->nombre,movi->movimiento,pokeJugador1->nombre,pokeJugador1->stats->hp);
    ...
    i_jug1++;
      if (i_jug1>2){
        ...
        printf("====CAMBIO DE POKEMON====\n");
        printf("Entra %s del Jugador 1 al combate.\n",equipoJugador1[i_jug1]);
        }
    }
    ...
    i_jug2++;
      if (i_jug2>2){
        ...
        printf("====CAMBIO DE POKEMON====\n");
        printf("Entra %s del Jugador 2 al combate.\n",equipoJugador2[i_jug2]);
        }
    }
  }
}//Cierra funcion
...
void mostrarMapaPokemon (Map * mapaPokemon){
  Pokemon * poke = firstMap(mapaPokemon); printf("nombre|tipo1|tipo2|hp|atk|def|atkesp|defesp|vel|legendary|mov1|mov2|mov3|mov4\n");
  while (poke!=NULL){
...
printf("|normal|lucha|volador|veneno|tierra|roca|bicho|fantasma|acero|fuego|agua|planta|electrico|psiquico|hielo|dragon|siniestro|hada\n");
...
void mostrarMapaMovimientos (Map * mapaMovimientos){
  ...
  printf("Nombre | Tipo | Daño | Precision | Fisico o Especial\n");
  while (mov!=NULL){
    printf("»%s, %s, %d, %d, %s \n",mov->movimiento,mov->tipo,mov->daño,mov->precision,mov->tipo_daño);
